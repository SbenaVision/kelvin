...
**Live-API results on an 8-case venture-assessment corpus (2026-04-24, `kelvin-eval==0.2.1`).** Running Kelvin against the live Venture Assessment API (`business-nudge.lovable.app`) on an 8-case corpus spanning SaaS, marketplace, fintech, hardware, content, AI-tools, local-services, and biotech verticals produced aggregate **Invariance 0.836, Sensitivity 0.207, K=0.957** on **n=7 effective cases × 77 perturbations (75 successful, 2 transient wrapper timeouts)**. One case (`biotech_diagnostics`) saw its baseline call fail with a non-zero exit from the API — cause indeterminate from the wrapper logs; likely an upstream transient or content moderation — which dropped its 11 perturbations from Phase 2 and reduced the effective sample from 8 to 7 cases. Full run report: `runs_0.2.1/real/kelvin/report.json`. The earlier n=2 pilot (Invariance 0.70, Sensitivity 0.50 across FreakingGenius and Envelop with 14 perturbations, 2 transient failures) is retained as §4's illustrative terminal output but is now superseded by the 8-case numbers as the paper's live-API anchor. The Envelop reorder position bias reported in §5.1 remains the canonical cited failure-mode discovery from the original pilot; the 2026-04-24 run did not re-surface an identical case because the corpus changed.

**Live-API results under full v0.3.0 scope (n=8 corpus, noise_floor replications=5, all Pillar 3 families + swap_condition enabled, 2026-04-24).** All 8 baselines succeeded; all 8 cases produced measurable σ_c. The run took 190.5 minutes wall-clock and executed 281 successful perturbations + 7 failed (VA-API transient errors). `swap_condition` produced 0 perturbations because the VA corpus governing types (`money`, `alternative`) are plain descriptive prose without the "requires: …" / state-phrase / details structure `swap_condition` requires — Pillar 2 decomposition is demonstrated on a separate gate-rule corpus in §5.4.

Pillar 1 calibration: Inv_raw = 0.869, Sens_raw = 0.177, K_raw = 0.955. η = 0.124, computed from 5 replays per case. **Inv_cal = 0.850, Sens_cal = 0.060, K_cal = 1.090.** K_cal is measurable but by a narrow margin (η < 1 − Inv_raw by 0.007). Sens_cal at 0.060 shows weak but non-zero rule-tracking residual above the stochasticity floor — a softer reading than an earlier narrower-sample run's Sens_cal ≈ 0.001, within the combined envelope of η variation (−0.027 between runs) and API-side sensitivity drift. Drift table and analysis in `experiments/pillar1_va_api_v030.md`.

Pillar 3 mechanical sensitivity aggregate = 0.187 across 48 samples, dominated by numeric_magnitude (44 samples; comparator_flip 1; polarity_flip 3). The closed-list comparator/polarity families found few applicable sites on this corpus, consistent with its prose structure; numeric_magnitude carried the signal. Pillar 3 invariance probes (presentation-layer and rule-based rhetorical with structural constraints) contributed ~145 additional samples to Inv_raw, raising the invariance sample count from v0.2's 72 to 217 on this run — Inv_raw itself moved upward modestly, consistent with a competent LLM correctly ignoring surface formatting.

Cross-pipeline K_cal landscape: constant **1.000**, brittle (first-500-chars) **1.055**, VA API **1.090** (v0.3.0 run), stochastic peer-swap adversary **None** (unmeasurable; η = 0.149 exceeds its invariance gap). The VA API lands between brittle and the unmeasurable adversary on K_cal — modestly better than brittle after calibration on the broader Pillar 3 invariance pool, but still far from a K = 0 anchoring target. This is the §3.4 paired-signal argument in its 0.3.0 form: K_raw credited noise-driven output changes as invariance signal; K_cal withdraws that credit. A narrower-sample run with Pillar 1 alone produced K_cal = 1.206 on the same pipeline; both remain defensible point estimates on their respective sample compositions and are retained in `experiments/pillar1_va_api_final.md` and `experiments/pillar1_va_api_v030.md`.

### 5.4 Pillar 2 decomposition — grounded rule-based pipeline (Table 4)

The `swap_condition` operator is defined for governing units whose body follows the "requires: &lt;condition list&gt;. &lt;state phrase&gt;. &lt;details&gt;" shape — a structure clean enough for a regex parser to identify. The VA tryout corpus does not use this structure (its governing types are prose pricing and alternatives descriptions), so the Pillar 2 decomposition is demonstrated here on the gate-rule corpus at `cases/` in the Kelvin repository, against the deterministic rule-based grounded stand-in from `experiments/tier3/pipelines/grounded.py`. Zero LLM spend; fully reproducible.

**Table 4 — Pillar 2 decomposition on the 6-case gate-rule corpus, grounded pipeline, seed=0:**

| Metric | Value |
|---|---:|
| Inv_raw | 0.852 |
| Sens(swap_content) (v0.2 raw swap) | **0.667** |
| Sens(swap_condition) ≈ Rule_Effect | **0.000** |
| Content_Effect = Sens(swap_content) − Sens(swap_condition) | **0.667** |
| Clean-parse rate (gate_rule structure) | **100% (6/6 cases)** |

The grounded pipeline's apparent raw-swap sensitivity is entirely Content_Effect. When the condition list is swapped but the state phrase and justification details are preserved, the pipeline's decision does not move on any of the 6 cases. When the whole gate_rule unit is swapped — the v0.2 operation that mixes rule-text change with factual-state change — the decision moves on 4 of 6 cases, yielding Sens_raw = 0.667.

This is the §3.3 content-leakage limitation made quantitative. The v0.2 raw-swap sensitivity of 0.667 on this pipeline was read in the v0.2 paper as "the grounded pipeline tracks the gate rule" and cited as evidence of evidence-tracking behavior. The Pillar 2 decomposition reveals the grounded pipeline is not reading the condition list at all — its routing keys off the state phrase ("All conditions are met") and content tokens (ARR, annual revenue, paying subscribers) in the details section. Raw-swap sensitivity credited this content-keyed routing as rule-tracking.

The clean-parse rate of 100% on this corpus is well above the 80% gate set in `FALLBACKS.md` before results arrived. Pillar 2 is a corpus-specific diagnostic: it applies cleanly to governing units with a structured condition/state partition and correctly emits zero perturbations on prose-based governing types. See `experiments/pillar2_decomposition_demo/README.md` for per-case breakdown and reproduction command.

### 5.5 Pillar 3 — presentation-layer invariance + mechanical sensitivity

Original Pillar 3 scope targeted rhetorical perturbation families (hedge injection, discourse markers, meta-commentary, etc.) and hinged on a human-labeling validation study to confirm each family was meaning-preserving. In v0.3.0 the validation study was dropped in favor of rule-based perturbations whose invariants hold *by construction* — positional rules, closed template lists, and structural constraints that make the generator's meaning-preservation claim trivially true rather than empirically validated.

Pillar 3 ships eleven families in two groups:

**Presentation-layer invariance (four families, strictly rule-based):**
- `whitespace_jitter` — blank-line jitter, trailing whitespace; tokens unchanged.
- `punctuation_normalize` — curly↔straight quotes, em-dash↔double-hyphen, ellipsis; orthographic only.
- `bullet_reformat` — `-`↔`*`, `1.`↔`1)`; list markers only.
- `non_governing_duplication` — verbatim sentence duplicate inside a non-governing section; the duplicated assertion was already present, so truth-conditional meaning is preserved (the pragmatic emphasis effect on non-decision-relevant content is a legitimate diagnostic signal, not a false positive).

**Rhetorical invariance (four families, rule-based with structural constraints):**
- `hedge_injection` — hedges before determiner-headed noun phrases only; never near negations or numeric tokens; never in governing sections.
- `politeness_injection` — closed-list imperative-verb rewrites ("Verify X" → "Please, verify X"); no effect on declarative prose.
- `discourse_marker_injection` — non-logical connectives only (Additionally, Furthermore); no however/therefore/nonetheless.
- `meta_commentary_injection` — framing phrases inserted only in non-governing sections.

**Mechanical sensitivity (three families, closed-list rule-based):**
- `numeric_magnitude` — multiplies numbers in governing sections by {2, 5, 10, 100}; four variants per case per governing type.
- `comparator_flip` — closed, hand-validated pair list ("at least" ↔ "at most", "more than" ↔ "less than", etc.).
- `polarity_flip` — closed, hand-validated antonym list ("approved" ↔ "denied", "met" ↔ "unmet", etc.).

On the VA API run, presentation-layer + rhetorical families contributed ~145 samples to the invariance pool. Their aggregate effect moved Inv_raw upward by ≈0.044 vs a Pillar-1-only sample composition — consistent with a competent LLM correctly being invariant to orthographic and rhetorical surface changes in non-governing content. Mechanical sensitivity measured 0.187, predominantly driven by numeric_magnitude (44 samples). Per §6 we note the rhetorical families trade structural simplicity for partial coverage of the rhetorical-sensitivity failure mode; a reviewer can cleanly separate what the rule-based construction protects (polarity, numeric literals, governing-section content) from what it does not protect (epistemic strength in non-governing prose, when a hedge lands). The v3 paper does not claim full meaning-preservation on the rhetorical families; it claims the rule-based positional constraints protect the specific invariants listed and surface a measurable signal from pipelines that are hypersensitive to rhetorical framing in non-governing content.
...
**Empirical scope is the 6-case Table 3 suite with a rule-based grounded stand-in plus the 8-case live-API run.** The Table 3 experiment (§5.3) demonstrates the paired-signal separation on a common suite, reproducibly and with zero LLM spend. The grounded column is a deterministic rule-based pipeline, not an LLM-backed production pipeline; absolute magnitudes do not calibrate across pipeline types. The 2026-04-24 live-API run on the 8-case venture-assessment corpus (§5.3) shipped the first larger-N anchor on a non-synthetic pipeline and surfaced concrete transient failure modes (one baseline dropped, two perturbation timeouts in a 77-perturbation run); the earlier n=2 Envelop pilot remains referenced as the discovery point for the position-bias failure-mode anecdote in §5.1.
