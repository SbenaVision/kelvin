...
The live-API numbers above are reported as K_raw and are not noise-calibrated. Pillar 1 of v0.3.0 (noise-floor calibration, `K_cal`) is expected to show that a meaningful fraction of the 0.164 invariance gap is LLM stochasticity rather than presentation-reactivity. Pillar 2 (`swap_condition` + decomposition) is expected to clarify how much of the 0.207 sensitivity reflects rule-tracking vs content leakage. §6 and §7 list these as named limitations of the current v1 signal; the v0.3.0 paper revision supersedes this paragraph with calibrated numbers.

**Partial Pillar 1 measurement (n=4 of 8 cases, 2026-04-24).** A first B6 noise-floor run against the same 8-case VA corpus completed 4 cases' baseline + 10-replay measurement before being silently killed by the OS mid-fifth-case (see `FAILURES.md` for the post-mortem and `experiments/pillar1_va_api_partial.md` for the raw per-case σ_c values). Partial η = **0.138** across `ai_dev_tool` (σ_c=0.172), `biotech_diagnostics` (0.098), `content_solo_creator` (0.098), and `fintech_compliance` (0.185). Projected onto the 2026-04-24 K_raw numbers above: **Inv_cal = 0.810, Sens_cal = 0.080, K_cal ≈ 1.110**. K_cal is measurable (η=0.138 < 1-Inv_raw=0.164) but lands *worse* than the brittle first-500-chars benchmark (K=1.055) under fair comparison — the VA API's raw-signal advantage over brittle was mostly LLM stochasticity getting credited as invariance. This partial finding is consistent with the §3.4 paired-signal argument: K_raw gives noise-driven output changes credit for looking like sensitivity; K_cal withdraws that credit. The n=4 caveat is load-bearing — the remaining 4 cases could shift η by ±0.02-0.05 and either keep K_cal measurable or push it to `None` (unmeasurable). A full n=8 verification run is scheduled for v0.3.0-rc1 before paper submission and will supersede the partial numbers here.
