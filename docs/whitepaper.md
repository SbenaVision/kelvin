...
**Final Pillar 1 measurement (n=8 corpus, n=6 effective σ_c, 2026-04-24).** The v0.3.0-rc1 verification run on the same 8-case VA corpus completed cleanly with all 8 baselines successful. Six cases produced full 10-replay σ_c measurements: `ai_dev_tool` 0.185, `biotech_diagnostics` 0.147, `content_solo_creator` 0.123, `fintech_compliance` 0.156, `hardware_climate` 0.168, `local_services_app` 0.129. The final two cases (`marketplace_consumer`, `saas_b2b_early`) had all 9 replays fail with exit-1 within a ~100-second window at the end of the baseline+replay phase — consistent with VA-API rate-limiting or transient infrastructure degradation after ~55 minutes of sustained traffic. The `retry_policy` fired correctly (29 `retry_giving_up` events across the run) but could not recover. `scorer.sigma_c()` returns `None` for cases with <2 replays and `aggregate()` drops those from η, so final η is the mean of 6: **η = 0.151**.

Calibration: Inv_raw = 0.824, Sens_raw = 0.152, K_raw = 1.023. With η = 0.151 and 1 − Inv_raw = 0.175, K_cal is measurable by a narrow 0.024 margin. **Inv_cal = 0.793, Sens_cal = 0.001, K_cal = 1.206.** The Sens_cal collapse to effectively zero is the load-bearing finding: governing-unit swaps did not move the VA API's decision above its own stochasticity floor. The 0.152 raw sensitivity was noise, not rule-tracking. Separately, the VA API's raw invariance (0.824) lost 0.031 units of credit to calibration — a portion of what K_raw attributed to "the pipeline staying calm when nothing important changes" was the LLM drifting between calls.

Cross-pipeline K_cal on this corpus with noise floor enabled: constant **1.000**, brittle (first-500-chars) **1.055**, VA API **1.206**, stochastic peer-swap adversary **None** (unmeasurable — η = 0.149 exceeds its invariance gap). **The VA API scores worse than the brittle first-500-chars pipeline under fair noise-calibrated comparison**, despite scoring better on raw K. This is the §3.4 paired-signal argument reaching its full form on a live LLM pipeline: K_raw credited noise-driven output changes as invariance signal; K_cal withdraws that credit and separately reveals the sensitivity axis as essentially non-operational. See `experiments/pillar1_va_api_final.md` for the per-case σ_c table, the partial-vs-final drift analysis, and pointers to the run artifacts.

Pillar 2 (`swap_condition` + decomposition) is expected to clarify whether any of the remaining ~0.001 Sens_cal reflects rule-tracking or is purely content-leakage noise, and what Content_Effect looks like on a pipeline whose Rule_Effect is already near the floor. Pillar 3 (validated intra-slot probes) is expected to expose whether the non-governing content leakage documented in §3.3 is quantitatively significant here. §6 and §7 list these as the remaining load-bearing questions for v0.3.0.
